	...
	if IsValidJob(sNotificationData['job']) and CheckOnDuty() then
			if not disableNotis then
				if sNotificationData.origin ~= nil then
					if sNotificationData.dispatchCode == "911" then
						TriggerEvent("chatMessage", '911', 1, sNotificationData.information, "dispatch")
					elseif sNotificationData.dispatchCode == "311" then
						TriggerEvent("chatMessage", '311', 1, sNotificationData.information, "dispatch")
					end
					SendNUIMessage({
						update = "newCall",
