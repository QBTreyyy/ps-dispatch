...
local function VehicleTheft(vehicle)
    ...
    TriggerServerEvent("dispatch:server:notify", {
        ...
        job = {"leo", "police", "bcso", "sdso","sasp","saspr","pbso"}
    })
end
...
local function InjuriedPerson()
    ...
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify", {
        dispatchcodename = "InjuriedPerson", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-69",
        firstStreet = locationInfo,
        gender = gender,
        model = nil,
        ...
        job = {"leo", "police", "bcso","ems","ambulance", "sdso","sasp","saspr","pbso"} -- type or jobs that will get the alerts
    })
end
...
local function DeceasedPerson()
    ...
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify", {
        dispatchcodename = "DeceasedPerson", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-69",
        firstStreet = locationInfo,
        gender = gender,
        model = nil,
...
local function StoreRobbery(camId)
    ...
    TriggerServerEvent("dispatch:server:notify", {
        ...
        job = {"leo", "police", "bcso", "sdso","sasp","saspr","pbso"} -- type or jobs that will get the alerts
    })
end
...
local function Reinforcement()
    ...
    local locationInfo = getStreetandZone(currentPos)
    local callsign = QBCore.Functions.GetPlayerData().metadata["callsign"]
    print('Player:' .. plyData.charinfo.firstname)
    TriggerServerEvent("dispatch:server:notify", {
        ...
        job = {"leo", "police", "bcso","ems","ambulance", "sdso","sasp","saspr","pbso"} 
        -- job = { "police", "bcso", "lspd", "sast", "sasp", "doc", "sapr", "pa", "sdso", "pbso", "bcso", "lspd", "sast", "sasp", "doc", "sapr", "pa", "sdso", "pbso" } -- jobs that will get the alerts
    })
end

exports('Reinforcement', Reinforcement)

RegisterNetEvent("ps-dispatch:client:reinforcement", function ()
    Reinforcement()
end)
...
local function CustomAlert(data)
    ...
    local job = {"leo", "police", "bcso", "sdso","sasp","saspr","pbso"}
...
local function ATMRobbery(camId)
    ...
    TriggerServerEvent("dispatch:server:notify", {
        ...
        job = {"leo", "police", "bcso", "sdso","sasp","saspr","pbso"} -- jobs that will get the alerts
    })
end
